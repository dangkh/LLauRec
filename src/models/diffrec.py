...
class ConditionalUNet(nn.Module):
    ...
    """
    def __init__(self, in_dims, out_dims, emb_size = 16, time_type="cat", norm=True, act_func='tanh', dropout=0.5):
        ...
        self.guidanceEmb = nn.Linear(384, self.in_dims[0])
        self.init_weights()
...
class GaussianDiffusion(nn.Module):
    def __init__(self, noise_schedule, noise_scale, noise_min, noise_max,\
    ...
    def training_losses(self, model, x_start, reweight=False, guidance = None):
        ...
        _predict_xstart_ = self._predict_xstart_from_eps(x_t, ts, model_output)
        if reweight == True:
            weight = (1 - self.alphas_cumprod[ts]) / (2 * (1-self.alphas_cumprod_prev[ts]**2) * (1-self.betas[ts]))
            weight = torch.where((ts == 0), 1.0, weight)
            likelihood = mean_flat((x_start - _predict_xstart_)**2 / 2.0)
        ...
        terms["pred_xstart"] = _predict_xstart_
...
if __name__ == "__main__":
    ...
    # Initialize model
    model = ConditionalUNet(in_dims=mlp_in_dims, out_dims=mlp_out_dims).to(device)
    ...
    ddpm = GaussianDiffusion(noise_schedule, noise_scale, noise_min, noise_max, steps, device).to(device)
    ...
    for epoch in range(20):
        # Dummy data
        loss = ddpm.training_losses(model, cid, True, temb)["loss"].mean()
    ...
    generated_cid = ddpm.p_sample(model, torch.randn(4, emb_dim).to(device), steps, sampling_noise=True, guidance=test_temb)