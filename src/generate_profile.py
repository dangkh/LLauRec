...
# -------- read JSON Lines --------
	...
	metaDF = pd.read_csv(file_path, sep="\t", usecols=['userID', 'itemID', 'x_label'])
	metaDF['userID'] = metaDF['userID'].astype(int)
	metaDF['itemID'] = metaDF['itemID'].astype(int)
	# metaDF = pd.DataFrame(metaDF)
	num_users = metaDF['userID'].nunique()
	print(num_users)

	users_by_label = {
    l: set(metaDF[metaDF['x_label'] == l]['userID'].unique())
    for l in [0, 1, 2]
	}
	...
	meta_items = set(metaDF['itemID'].unique())
